...
export default function Main(props) {
	...
	const [traingTime, setTraingTime] = useState(0);
	const [dday, setDday] = useState(0);

	const handleSubmit = (event) => {
		event.preventDfault();
		setDday(Math.ceil(10000 / traingTime));
	}
		...
		<main>
			...
			<form className="cont-input" onSubmit={handleSubmit}>
				...
				<p className="txt-time">그래서 앞으로 매일 하루에
					<input type="number" required value={traingTime} onChange={(event) => setTraingTime(event.target.value)}/>시간씩 훈련할 것이다.
				</p>
